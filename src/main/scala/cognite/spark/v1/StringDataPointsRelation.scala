...
import cognite.spark.v1.Constants
...
class StringDataPointsRelationV1(config: RelationConfig)(override val sqlContext: SQLContext)
  ...
  private def limitForCall(nPointsRemaining: Option[Int]) =
    (nPointsRemaining, config.batchSize) match {
      ...
      case (remaining @ Some(_), Some(batchSize)) => remaining.map(_.min(batchSize))
    ...
    if (lowerLimit.toEpochMilli >= upperLimit.toEpochMilli || nPointsRemaining.exists(_ <= 0)) {
      ...
      queryPoints
        ...
        .flatMap {
          case Nil => allPoints
          case points @ _ :+ last =>
            val newLowerLimit = last.timestamp.plusMillis(1)
            val accumulatedPoints = allPoints.map { accumulated =>
              val pointsFromResponse = nPointsRemaining match {
                case None => points
                case Some(maxNumPointsToInclude) => points.take(maxNumPointsToInclude)
              }
              accumulated ++ pointsFromResponse
            }
            getAllDataPoints(
              id,
              newLowerLimit,
